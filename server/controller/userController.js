...
exports.signup = catchAsync(async (req, res, next) => {
  ...
  //   <Welcome name={user.email}></Welcome>
  // );
  // new Email(messageHtml, user.email).sendWelcome();
...
exports.addorderApi = catchAsync(async (req, res, next) => {
  ...
  const productdetail = await Orderschema.create({
    orderno: orderno,
    ...req.body,
  });
...
const Userrouteschema = require("../model/userschema");
...
exports.logout = catchAsync(async (req, res, next) => {
  MongoClient.connect(process.env.DATABASE, function (err, client) {
    ...
    if (req.session.user) {
      ...
      const datas = db
        .collection("mySessions")
        .deleteOne({ _id: req.session.admin._id });
    }
